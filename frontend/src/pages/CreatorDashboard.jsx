...
export const CreatorDashboard = () => {
    ...
    const handleExportPDF = async (event) => {
        try {
            ...
            const creatorData = {
                name: profile.name,
                totalFollowers: profile.platforms.instagram?.followers + (profile.platforms.youtube?.followers || 0),
                projectsCompleted: 12,
                avgEngagement: parseFloat(profile.stats.engagement_rate),
                metrics: {
                    totalReach: profile.stats.total_views,
                    engagementRate: parseFloat(profile.stats.engagement_rate),
                    contentPosted: 45,
                    avgViews: Math.round(profile.stats.total_views / 45)
                },
                platforms: Object.entries(profile.platforms).reduce((acc, [p, d]) => {
                    acc[p] = { followers: d.followers, engagement: profile.stats.engagement_rate, posts: 15 };
                    return acc;
                }, {}),
                revenue: {
                    thisMonth: 8500 * (profile.stats.total_views / 892340),
                    lastMonth: 7200 * (profile.stats.total_views / 892340),
                    yearlyTotal: 95000 * (profile.stats.total_views / 892340),
                    monthlyProjects: 3,
                    lastMonthProjects: 2,
                    yearlyProjects: 12
                }
            ...
            case 'analytics':
                return (
                    <div className="space-y-3">
                        { }
                        <Card className="border-l-4 border-yellow-500 bg-yellow-50 dark:bg-slate-800">
                            <CardContent className="p-3 text-sm">
                                <strong>What's working:</strong>{' '}
                                Short-form content on Instagram is driving the highest engagement this month, outperforming static posts.
                            </CardContent>
                        </Card>
                    ...
                    <div className="h-full flex flex-col gap-3">
                        ...
                        <div className="flex-1 overflow-y-auto pr-2">
                            <div className="max-w-4xl mx-auto">
                                <RevenueMetrics platformFilter={selectedPlatform} />
                            </div>
                        </div>
                    </div>
                )
